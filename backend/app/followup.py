...
def build_followup_questions(parsed: ParsedDailyLog) -> list[str]:
    ...
    if (
        parsed.workout
        and not parsed.workout.exercises
        and not _is_cardio(parsed.workout.workout_type)
    ):
    ...
    if (
        parsed.workout
        and parsed.workout.duration_min is not None
        and parsed.workout.intensity is None
    ):
    ...
    if parsed.nutrition and any(
        item.estimated and item.calories is not None for item in parsed.nutrition
    ):
        questions.append(
            "Want to replace the estimated calories with actual calories for any meal?"
        )
...
def _is_cardio(workout_type: str | None) -> bool:
    ...
    return any(
        marker in lower for marker in ("run", "running", "cardio", "bike", "cycling", "swim")
    )
