...
class FakeWorkflow:
    async def process_text(self, text: str, *, source: str, entry_date: date | None) -> WorkflowResult:
    ...
    async def log_text(self, text: str, *, source: str, entry_date: date | None) -> WorkflowResult:
...
class AgentApiTests(TestCase):
    ...
    def test_agent_log_mode_returns_conversation_metadata(self) -> None:
        ...
        self.assertEqual(payload["assumption"], "Log mode stores this as a daily record, even if it looks like a command.")
    ...
    def test_agent_auto_mode_can_return_non_log_reply(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload["status"], "ignored_non_logging_reply")
